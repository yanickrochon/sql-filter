...
module.exports = new class PostgresAdapter extends Adapter {
  ...
  build(data, options) {
    ...
    options.placeholderPrefix = 'placeholderPrefix' in options ? options.placeholderPrefix : this.DEFAULT_PLACEHOLDER_PREFIX;

    if (typeof options.valueWrapper !== 'function') {
    ...
    } else if (typeof options.placeholderPrefix !== 'string') {
      throw new TypeError('Placeholder prefix is not a string');
    }
...
function buildExpression(self, fieldPrefix, field, value, options) {
  ...
  return operatorHandler.call(self, fieldPart, fieldOperator, fieldArgs.map(function (arg) { return options.placeholderPrefix + String(arg); }), fieldArgSuffix, options);
}
