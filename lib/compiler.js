...
function processTokens(compiled, tokens) {
  ...
  for (let i = 0, iLen = tokens.length; i < iLen; ++i) {
    ...
    if (tokens[i].argument) {
      fieldValue.argument = '@' + tokens[i].argument;
    }
  }
...
function collect(tree, cache, tokens) {
  if (typeof tree === 'string') {
    ...
    if (tree.options) {
      tree.options && tree.options.operator && (tokens[0].operator = tree.options.operator);
      tree.options && tree.options.argument && (tokens[0].argument = tree.options.argument);
    }
...
function isFieldValueEqual(a, b) {
  return (a.operator === b.operator) && (a.argument === b.argument);
}
